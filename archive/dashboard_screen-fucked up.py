...
class DashboardScreen(QWidget):
    ...
    def __init__(self, user_data: Dict[str, Any]):
        ...
        # NIEUW/FIXED ATTRIBUTEN: Essentieel voor de badge update
        self.management_layout: Optional[QHBoxLayout] = None
        self.verlof_button_widget: Optional[QWidget] = None

        self._setup_ui()

    def _setup_ui(self) -> None:
        """Stel de hoofd UI op - Gecorrigeerde tab structuur."""
        ...
        # Controleer of de gebruiker een planner is
        is_planner = self.user_data.get('rol') == 'planner'

        # Voeg de tabs toe in de correcte volgorde:

        # 1. PLANNING TAB (ALLEEN VOOR PLANNERS)
        if is_planner:
            # Assumptie: U heeft een methode _get_planning_tab()
            self.tabs.addTab(self._get_planning_tab(), "Planning")

            # 2. BEHEER TAB (ALLEEN VOOR PLANNERS)
        if is_planner:
            # Hierin zit de Verlof Goedkeuring knop met de badge.
            self.tabs.addTab(self._get_management_tab(), "Beheer")

        # 3. HR-INSTELLINGEN TAB (ALLEEN VOOR PLANNERS)
        if is_planner:
            # Assumptie: U heeft een methode _get_hr_settings_tab()
            self.tabs.addTab(self._get_hr_settings_tab(), "HR-instellingen")

        # 4. PERSOONLIJK TAB (ALTIJD)
        self.tabs.addTab(self._get_personal_tab(), "Persoonlijk")
    ...
    def refresh_verlof_badge(self):
        count = self.get_pending_leave_count()
        self.update_verlof_badge_ui(count)
...
class NotitieNaarPlannerDialog(QDialog):
    ...
    def init_ui(self) -> None:
        ...
        self.notitie_edit.setPlaceholderText(
            "Bijv: Heb een afspraak om 14u, kan niet voor late shift. "
        ...
        info.setWordWrap(True)
        info.setStyleSheet(f"color: {Colors.TEXT_SECONDARY}; font-style: italic; padding: {Dimensions.SPACING_SMALL}px;")
        layout.addWidget(info)
        ...
        button_box.button(QDialogButtonBox.StandardButton.Save).setText("Opslaan")
        button_box.button(QDialogButtonBox.StandardButton.Cancel).setText("Annuleren")
