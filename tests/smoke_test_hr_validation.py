...
def test_planning_validator_init():
    ...
    try:
        ...
        if not user:
            print("✗ Geen actieve gebruikers gevonden in database")
            return None

        print(f"✓ Test gebruiker: {user['volledige_naam']} (ID: {user['id']})")
        ...
        print(f"✓ PlanningValidator geïnstantieerd")
...
def test_hr_config_loading(validator: PlanningValidator):
    ...
    try:
        ...
        print(f"✓ HR config geladen: {len(hr_config)} regels")
...
def test_shift_tijden_loading(validator: PlanningValidator):
    ...
    try:
        ...
        print(f"✓ Shift tijden geladen: {len(shift_tijden)} codes")
...
def test_planning_data_loading(validator: PlanningValidator):
    ...
    try:
        ...
        print(f"✓ Planning data geladen: {len(planning)} dagen")
...
def test_constraint_checker_init(validator: PlanningValidator):
    ...
    try:
        ...
        print(f"✓ ConstraintChecker geïnstantieerd")
        ...
        print(f"✗ ConstraintChecker error: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_validate_all(validator: PlanningValidator):
    ...
    try:
        ...
        print(f"✓ validate_all() uitgevoerd")
        print(f"✓ Resultaat: {len(violations_dict)} regel checks")
        ...
        print(f"\n✓ Totaal: {total_violations} violations gevonden")

        return violations_dict

    except Exception as e:
        print(f"✗ validate_all() error: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def test_validate_shift(validator: PlanningValidator):
    ...
    try:
        ...
        print(f"✓ validate_shift() uitgevoerd")
        print(f"✓ Resultaat: {len(violations)} violations")

        if violations:
            for v in violations:
                print(f"  ⚠ {v.beschrijving}")
        else:
            print(f"  ✓ Geen violations (shift is OK)")

        return True

    except Exception as e:
        print(f"✗ validate_shift() error: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_ui_helpers(validator: PlanningValidator):
    ...
    try:
        ...
        print(f"✓ get_violation_level({test_datum}): '{level}'")

        # Test get_violations_voor_datum
        violations = validator.get_violations_voor_datum(test_datum)
        print(f"✓ get_violations_voor_datum({test_datum}): {len(violations)} violations")
...
def main():
    ...
    if validator is None:
        print("\n✗ Cannot continue - validator init failed")
        return
    ...
    results.append(("Planning Data Loading", planning_count > 0))
